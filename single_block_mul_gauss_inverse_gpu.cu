...
static
int single_sm_inverse_gauss_gpu(__DATA_TYPE* out, int size, int my_np, float& elapse_time) {
...
static
int more_sm_inverse_gauss_gpu(__DATA_TYPE* out, int size, int my_np, float& elapse_time) {
...
int my_single_block_mul_gauss_inverse_gpu(__DATA_TYPE* out, int size , int my_np, float& elapse_time) {
	...
	if (my_np >= 1 && my_np < 1024) {
		return single_sm_inverse_gauss_gpu(out, size, my_np,elapse_time);
	}
	else {
		return more_sm_inverse_gauss_gpu(out, size, my_np, elapse_time);
	}
}